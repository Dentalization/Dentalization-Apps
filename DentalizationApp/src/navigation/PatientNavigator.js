...
import DiagnosisPatient from '../screens/patient/diagnosisPatient/DiagnosisPatient';
...
const CustomTabBar = ({ state, descriptors, navigation }) => {
  const tabs = [
    ...
    // Ganti icon tengah: dari "camera" -> "tooth" (AI deteksi gigi)
    { name: ROUTES.PATIENT_CAMERA, label: '', icon: 'tooth', type: 'MaterialCommunityIcons', isCenter: true },
  ...
  return (
    ...
    >
      {tabs.map((tab, index) => {
        ...
        if (tab.isCenter) {
          // Tombol tengah dengan ikon gigi + badge "AI"
          return (
            <TouchableOpacity
              key={index}
              style={{
              ...
              accessibilityLabel="AI Deteksi Gigi"
            >
              <LinearGradient
              ...
              >
                ...
                {/* Badge "AI" kecil di pojok kanan atas */}
                <View
                  style={{
                    position: 'absolute',
                    right: 6,
                ...
                >
                  <Text style={{ fontSize: 10, fontWeight: '700', color: '#6D28D9' }}>AI</Text>
                </View>
              </LinearGradient>
            </TouchableOpacity>
          );
        ...
        return (
          <TouchableOpacity
            key={index}
            style={{ alignItems: 'center', justifyContent: 'center', flex: 1, paddingVertical: 12, paddingHorizontal: 8 }}
            onPress={onPress}
          ...
          >
            <View
              style={{
                alignItems: 'center',
                justifyContent: 'center',
                backgroundColor: isFocused ? 'rgba(139, 92, 246, 0.1)' : 'transparent',
                borderRadius: 16,
                paddingVertical: 8,
